...
def test(data,
    ...
    model = load_model(weights, device, half_precision, (1, 3, imgsz, imgsz)) if model is None else model
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            t = time_synchronized()
            out = non_max_suppression(out, conf_thres, iou_thres, labels=lb, multi_label=True, agnostic=single_cls, kpt_label=kpt_label, nc=1, nkpt=17)
